...
class Pagination
{
    ...
    public function get()
    {
        ...
        $html = '<div class="pagination clearfix"><nav class="pagination__nav right clearfix">';
        # Генерируем ссылки
        for ($page = $limits[0]; $page <= $limits[1]; $page++) {
            # Если текущая это текущая страница, ссылки нет и добавляется класс active
            if ($page == $this->current_page) {
                $links .= '<span class="pagination__page pagination__page--current">' . $page . '</span>';
            } else {
    ...
    private function generateHtml($page, $text = null)
    {
        ...
        return
                '<a href="' . $currentURI . $this->index . $page . '" class="pagination__page">' . $text . '</a>';
    }
